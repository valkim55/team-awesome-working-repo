  ...
  fetch(currentLocrequestUrl, { method: 'GET' }) //fetaching all realted area for current location
    ...
    .then((currentLocdata) => {
      for (var i = 0; i < currentLocdata.results.length; i++) {
        ...
        latitude = JSON.stringify(currentLocdata.results[i].locations[i].latLng['lat']); //getting the latitude of users current location
        longitude = JSON.stringify(currentLocdata.results[i].locations[i].latLng['lng']); //getting the longitude of users current location
      }
